...
export default function FoodPage() {
  ...
  const searchFood = async (q: string, c: string) => {
    ...
    setSearched(true)
    try {
      const res = await fetch(`/api/food?q=${encodeURIComponent(q.trim())}&city=${c.toLowerCase()}`)
      ...
      <div className="mx-auto max-w-3xl px-4 py-8">
        ...
        <div className="mb-4">
          ...
          <div className="flex gap-2 overflow-x-auto pb-2">
            {CITIES.map((c) => (
              <button key={c} onClick={() => { setCity(c); searchFood(query, c) }}
                className={`shrink-0 rounded-full border px-4 py-1.5 text-xs font-bold transition-all ${
                  city === c ? "border-[#FF6B35] bg-[#FF6B35]/15 text-[#FF9500]" : "border-white/10 text-white/50 hover:border-white/20 hover:text-white"
                }`}>
                {c}
              </button>
            ))}
        ...
        <div className="flex gap-3 mb-4">
          ...
          <button onClick={() => searchFood(query, city)} disabled={loading}
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 mb-6">
          {POPULAR.map((item) => (
            <button key={item} onClick={() => { setQuery(item); searchFood(item, city) }}
        ...
        {cheapest && !loading && (
          <div className="rounded-xl border border-[#00C875]/30 bg-[#00C875]/5 p-4 mb-6 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              {cheapest.image && <img src={cheapest.image} alt={cheapest.name} className="h-12 w-12 rounded-xl object-cover" />}
              <div>
                <div className="text-xs font-bold text-[#00C875] mb-0.5 uppercase tracking-wider">Cheapest in {currentCity}</div>
                <div className="font-semibold text-sm line-clamp-1">{cheapest.name}</div>
              </div>
            </div>
            <div className="text-right shrink-0">
              <div className="text-2xl font-extrabold text-[#00C875]">Rs.{cheapest.finalPrice ?? cheapest.price}</div>
        ...
        {!loading && results.length > 0 && (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            {results.map((item) => {
              ...
              const hasDiscount = item.finalPrice !== null && item.price !== null && item.finalPrice < item.price
              const ytQuery = item.restaurantName ? `${item.restaurantName} restaurant review` : `${item.name} food review`

              return (
                <div key={item.id} className={`rounded-2xl border p-4 transition-all ${isCheapest ? "border-[#00C875]/40 bg-[#00C875]/5" : "border-white/7 bg-[#0D0D1B] hover:border-white/15"}`}>
                  <div className="flex gap-3 mb-3">
                    {item.image ? (
                      <img src={item.image} alt={item.name} className="h-20 w-20 rounded-xl object-cover shrink-0" />
                    ) : (
                      <div className="h-20 w-20 rounded-xl bg-white/5 shrink-0 flex items-center justify-center text-3xl">🍽️</div>
                    )}
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 mb-1">
                        {item.isVeg !== undefined && (
                          <span className={`text-xs ${item.isVeg ? "text-green-400" : "text-red-400"}`}>{item.isVeg ? "🟢" : "🔴"}</span>
                        )}
                        {isCheapest && (
                      ...
                      {item.restaurantName && <p className="text-xs text-white/35 mt-0.5">{item.restaurantName}</p>}
                      {item.rating !== "N/A" && <p className="text-xs text-yellow-400 mt-1">⭐ {item.rating}</p>}
                    </div>
                  </div>

                  <div className="flex items-center gap-2 mb-3">
                    <span className={`text-2xl font-extrabold ${isCheapest ? "text-[#00C875]" : "text-white"}`}>Rs.{displayPrice}</span>
                    {hasDiscount && <span className="text-sm text-white/30 line-through">Rs.{item.price}</span>}
                    {hasDiscount && (
                  ...
                  <div className="text-xs text-white/40 mb-3">
                    <span className="text-[#FC8019] font-semibold">Swiggy</span>{" · "}⏱ {item.deliveryTime}
                  </div>

                  <div className="flex items-center gap-2">
                    <a href={item.link} target="_blank" rel="noopener noreferrer"
                      className={`flex-1 rounded-lg px-3 py-2 text-xs font-bold text-center hover:opacity-90 transition-opacity ${isCheapest ? "bg-gradient-to-r from-[#FF6B35] to-[#FF9500] text-white" : "bg-white/8 text-white/70"}`}>
                      Order
                    </a>
